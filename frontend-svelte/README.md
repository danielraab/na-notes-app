...
Svelte implementation of the NA Notes frontend. Talks to whichever backend
...
Same feature set as `frontend-react`, different stack and a different
color scheme/visual style — see [`docs/decisions/`](docs/decisions) for
why each choice was made.

## Stack

- Vite + Svelte 5 + TypeScript, using runes (`$state`/`$derived`/`$effect`)
  for reactivity — no Svelte stores except the tiny router.
- A hand-rolled ~60-line client-side router (`src/lib/router.ts`) instead
  of a routing library (`docs/decisions/0005-custom-router.md`).
- Plain CSS with custom-property design tokens (`src/app.css`) — no
  Tailwind (`docs/decisions/0006-plain-css-styling.md`).
- `marked` + `dompurify` for rendering and sanitizing note Markdown.
- `vitest` for unit tests, `svelte-check` for type/template checking,
  `oxlint` for linting the plain `.ts` modules.
...
src/auth/       # auth.svelte.ts: a runes-based singleton store exposing user/loading/login()/logout()
src/components/ # NoteCard, NoteGrid (infinite scroll), MarkdownEditor/View, ShareDialog, Header
src/hooks/      # noteFeed.svelte.ts: cursor-pagination state machine for the dashboard
src/lib/        # router.ts: the minimal History-API router
src/pages/      # DashboardPage, NoteEditorPage, PublicNotePage
src/utils/      # pure helpers (mention parsing/insertion, task-list parsing) — unit tested
```
...
npm run lint      # oxlint (plain .ts modules only, see docs/decisions/0001)
npx svelte-check  # type-check, including .svelte templates
npm run build     # svelte-check + tsc + production build
```

There is no component/E2E test suite yet; UI behavior was verified
manually against a mocked API (see
`docs/decisions/0003-manual-e2e-verification.md`).

## Docker

```bash
docker build -t na-notes-frontend-svelte --build-arg VITE_API_BASE_URL=http://localhost:8080/api .
docker run --rm -p 5173:80 na-notes-frontend-svelte
```
...
- All note Markdown is rendered via `marked` and then sanitized with
  `DOMPurify` before being injected as HTML (`MarkdownView.svelte`) —
  never trust markdown as safe HTML just because it came from our own API,
