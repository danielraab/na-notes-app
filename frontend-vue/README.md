...
Vue implementation of the NA Notes frontend. Talks to whichever backend
...
- Vite + Vue 3 (Composition API, `<script setup lang="ts">`) + TypeScript.
- `vue-router` for client-side routing.
- Plain CSS (custom-property design tokens in `src/app.css`) for
  styling — a warm amber palette, distinct from `frontend-react`'s violet
  Tailwind look and `frontend-svelte`'s teal serif look (see
  `docs/decisions/0006-css-styling.md`).
...
src/auth/        # auth composable (module-level reactive singleton): fetches /api/auth/me, exposes login()/logout()
src/components/  # NoteCard, NoteGrid (infinite scroll), MarkdownEditor/View, ShareDialog, Header
src/composables/ # useNoteFeed: cursor-pagination state machine for the dashboard
src/pages/       # DashboardPage, NoteEditorPage, PublicNotePage, NotFoundPage
src/router/      # vue-router route table
src/utils/       # pure helpers (mention parsing/insertion, task-list toggling) — unit tested
```
...
npm run lint     # oxlint
npx vue-tsc -b   # type-check (includes .vue templates)
...
```bash
docker build -t na-notes-frontend-vue --build-arg VITE_API_BASE_URL=http://localhost:8080/api .
docker run --rm -p 5173:80 na-notes-frontend-vue
```
...
- All note Markdown is rendered via `marked` and then sanitized with
  `DOMPurify` before being injected as HTML (`MarkdownView.vue`) — never
