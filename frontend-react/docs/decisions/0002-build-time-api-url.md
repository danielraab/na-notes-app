...
runtime `.env` reload for a static SPA build the way there is for the
Go backend.
...
- Trade-off: can't reuse one built frontend image across environments
  ...
  production deployment). If that changes, revisit with a small
  entrypoint script that writes a `config.js` from an env var before
  nginx starts.
